...
@section('page-title', 'Create Compliance Deadline')
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Create Compliance Deadline</h1>
        <a href="{{ route('admin.compliance-deadlines.index') }}"
            class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded">Cancel</a>
    </div>
    <form method="POST" action="{{ route('admin.compliance-deadlines.store') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <input type="text" name="deadline_name" value="{{ old('deadline_name') }}" required
            ...
            <div>
                ...
                <select name="deadline_type" id="deadline_type" required class="w-full border rounded px-3 py-2 text-sm"
                    ...
                    <option value="static" {{ old('deadline_type') == 'static' ? 'selected' : '' }}>Static Date</option>
                    <option value="dynamic" {{ old('deadline_type') == 'dynamic' ? 'selected' : '' }}>Dynamic Rule</option>
                </select>
            </div>
            <div>
            ...
            <div id="static_fields" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="fixed_month" value="{{ old('fixed_month') }}" min="1" max="12"
                        class="w-full border rounded px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Fixed Day (1-31)</label>
                    <input type="number" name="fixed_day" value="{{ old('fixed_day') }}" min="1" max="31"
                        class="w-full border rounded px-3 py-2 text-sm">
                </div>
            </div>
            <div id="dynamic_fields" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6"
                style="{{ old('deadline_type') == 'static' && !old('deadline_type') ? 'display:none' : '' }}">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Rule Label</label>
                    <input type="text" name="rule_label" value="{{ old('rule_label') }}"
                        class="w-full border rounded px-3 py-2 text-sm"
                ...
                <div>
                    ...
                    <select name="rule_type" required class="w-full border rounded px-3 py-2 text-sm">
                        <option value="fixed" {{ old('rule_type') == 'fixed' ? 'selected' : '' }}>Fixed</option>
                        <option value="days_after_formation"
                            {{ old('rule_type') == 'days_after_formation' ? 'selected' : '' }}>Days After Formation
                        </option>
                        <option value="days_after_fy_end" {{ old('rule_type') == 'days_after_fy_end' ? 'selected' : '' }}>
                            Days After FY End</option>
                        <option value="anniversary" {{ old('rule_type') == 'anniversary' ? 'selected' : '' }}>Anniversary
                        </option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Rule Days</label>
                    <input type="number" name="rule_days" value="{{ old('rule_days') }}" min="0"
                        class="w-full border rounded px-3 py-2 text-sm">
                </div>
            </div>
            <div>
