...
<div class="space-y-4">
    ...
    <div>
        ...
        <input type="text" name="short_description" id="short_description"
            value="{{ old('short_description', $industry?->short_description) }}" maxlength="255"
    ...
    <div>
        ...
        <textarea name="description" id="description" rows="12"
            ...
            placeholder="Detailed description with rich text...">{{ old('description', $industry?->description) }}</textarea>
        @error('description')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <input type="text" name="seo_title" id="seo_title" value="{{ old('seo_title', $industry?->seo_title) }}"
            maxlength="255"
    ...
    <div>
        ...
        <textarea name="seo_keywords" id="seo_keywords" rows="2"
            ...
            placeholder="Comma-separated keywords">{{ old('seo_keywords', $industry?->seo_keywords) }}</textarea>
        @error('seo_keywords')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <textarea name="seo_description" id="seo_description" rows="3"
            ...
            placeholder="Meta description for search engines">{{ old('seo_description', $industry?->seo_description) }}</textarea>
    ...
    <div>
        ...
        <input type="url" name="canonical_url" id="canonical_url"
            value="{{ old('canonical_url', $industry?->canonical_url) }}" maxlength="255"
    ...
    @if ($industry)
        <div>
            ...
            <select name="status" id="status"
                ...
                <option value="1" {{ old('status', $industry?->status) == 1 ? 'selected' : '' }}>Active
                </option>
                <option value="0" {{ old('status', $industry?->status) === 0 ? 'selected' : '' }}>Inactive
                </option>
            </select>
            @error('status')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
