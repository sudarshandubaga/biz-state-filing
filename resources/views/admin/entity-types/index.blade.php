...
@section('page-title', 'Entity Types Management')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.entity-types.index') }}" method="GET"
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-gray-900">Entity Types List</h3>
                <a href="{{ route('admin.entity-types.create') }}"
                    class="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700">
                    + Add New Entity Type
                </a>
            </div>

            <form action="{{ route('admin.entity-types.bulk-action') }}" method="POST" id="bulkForm">
                @csrf
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($entityTypes as $entityType)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <input type="checkbox" name="ids[]" value="{{ $entityType->id }}"
                                            class="row-checkbox h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded cursor-pointer">
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        @if ($entityType->icon)
                                            <i class="fas {{ $entityType->icon }} text-lg text-blue-600"></i>
                                        @else
                                            <span class="text-gray-400">-</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $entityType->name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-500">{{ $entityType->slug ?? 'N/A' }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $entityType->created_at->format('M d, Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <button type="button"
                                            onclick="deleteItem({{ $entityType->id }}, '{{ route('admin.entity-types.destroy', $entityType) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center text-gray-500">No entity types found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
