...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.affiliates.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Search</label><input type="text"
                        name="search" value="{{ request('search') }}" placeholder="Search by name, email, company..."
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Status</label><select name="status"
                ...
                <div class="flex items-end space-x-2"><a href="{{ route('admin.affiliates.index') }}"
                        class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</a><button
                        type="submit"
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-gray-900">Affiliates List</h3>
            ...
            <form action="{{ route('admin.affiliates.bulk-action') }}" method="POST" id="bulkForm">
                @csrf
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($affiliates as $affiliate)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $affiliate->company ?: 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $affiliate->email }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap"><span
                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{{ $affiliate->commission_priority }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $affiliate->current_load }}/{{ $affiliate->max_load }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap"><span
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('admin.affiliates.show', $affiliate) }}"
                                            ...
                                            title="View"><svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                        ...
                                        <button type="button"
                                            onclick="deleteItem({{ $affiliate->id }}, '{{ route('admin.affiliates.destroy', $affiliate) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="px-6 py-4 text-center text-gray-500">No affiliates found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="px-6 py-4 border-t border-gray-200">{{ $affiliates->links() }}</div>
            </form>
        </div>
    </div>
@endsection
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('bulkForm')?.addEventListener('submit', function(e) {
                ...
                if (action === 'delete' && !confirm('Delete selected affiliates?')) {
                    e.preventDefault();
                }
            });
