...
@section('content')
    <div class="min-h-screen flex items-center justify-center bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            ...
            <form class="mt-8 space-y-6" action="{{ route('admin.reset-password') }}" method="POST">
                @csrf
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" autocomplete="email" required readonly
                        class="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 text-gray-500 bg-gray-100 sm:text-sm"
                        value="{{ $email }}">
                </div>
