...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.states.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Search -->
                <div>
                ...
                <div>
                    ...
                    <select name="deadline_type"
                        ...
                        <option value="fixed" {{ request('deadline_type') == 'fixed' ? 'selected' : '' }}>Fixed</option>
                        <option value="anniversary" {{ request('deadline_type') == 'anniversary' ? 'selected' : '' }}>
                            Anniversary</option>
                        <option value="varies" {{ request('deadline_type') == 'varies' ? 'selected' : '' }}>Varies</option>
                    </select>
                </div>

                <div class="md:col-span-4 flex justify-end">
                    <a href="{{ route('admin.states.index') }}"
                        class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 mr-2">Clear</a>
                    <button type="submit"
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-gray-900">States List</h3>
            ...
            <form action="{{ route('admin.states.bulk-action') }}" method="POST" id="bulkForm">
                @csrf
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($states as $state)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $state->state_name }}</div>
                                        <div class="text-sm text-gray-500">{{ $state->country->country_name ?? 'N/A' }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-500">{{ $state->state_slug }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm text-gray-900">${{ number_format($state->filing_fee, 2) }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="px-2 py-1 text-xs font-semibold rounded-full
                                        @if ($state->deadline_type == 'fixed') bg-blue-100 text-blue-800
                                        @elseif ($state->deadline_type == 'anniversary') bg-green-100 text-green-800
                                        @else bg-yellow-100 text-yellow-800 @endif">
                                            {{ ucfirst($state->deadline_type) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="px-2 py-1 text-xs font-semibold rounded-full
                                        @if ($state->status) bg-green-100 text-green-800
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <button type="button"
                                            onclick="deleteItem({{ $state->id }}, '{{ route('admin.states.destroy', $state) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center text-gray-500">No states found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
